...
var timers = {};
ari.connect('http://knuckle.palaver.net:8088', 'brianc', 'getmeoutofhere', clientLoaded);
  ...
  function stasisStart(event, channel) {
    ...
    function answer() {
      ...
      channel.startSilence(function(err) {
        console.log('Doing some silence for a while')
        if (err) {
          throw err;
        }
      });
      // hang up the channel in 4 seconds
      var timer = setTimeout(hangup, 10000);
      timers[channel.id] = timer;
    }
